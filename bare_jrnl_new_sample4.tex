...
We vary the trigger location among low-, middle-, and high-frequency bands while holding all other settings fixed. Table~\ref{tab:freq_band_ablation} shows that low-frequency components consistently provide the strongest activation, whereas middle- and high-frequency triggers have near-zero ASR across the three models. High-frequency modifications are substantially less perceptible (PSNR $47.68$~dB and SSIM $0.9973$, versus $29.23$~dB and $0.9568$ for low frequencies) but fail to activate the learned backdoor reliably. Thus, low-frequency injection favors activation over visual similarity.
