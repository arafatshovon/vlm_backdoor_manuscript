...
% \IEEEPARstart{V}{ision-language} models (VLMs) expose a broad attack surface because malicious behavior can be introduced through their visual, textual, or cross-modal pathways. Backdoor attacks are particularly concerning: a compromised VLM can behave normally on benign inputs yet produce attacker-specified outputs when a trigger is present.
...
Unlike conventional vision backdoors, which typically associate a visual trigger with a target class, attacks on generative VLMs must control open-ended textual outputs. Recent studies show that poisoned VLMs can inject malicious text, alter visual concepts, or induce other attacker-controlled behaviors while preserving clean-input performance~\cite{Lyu2024TrojVLM,Lyu2025VLOOD,Yuan2025BadToken}, exposing a distinct security risk in multimodal generative systems.

Despite this progress, existing VLM backdoors predominantly use spatial, textual, or bimodal triggers. Spatial triggers, including localized patterns, patches, and image-level modifications, may leave recognizable visual or spatial characteristics. Frequency-domain attacks can instead exploit spectral information to construct less perceptible triggers in conventional vision models. However, their use for backdooring \emph{generative VLMs} remains largely unexplored. In particular, it is unclear whether spectral perturbations can establish an effective trigger-to-text association while preserving the visual semantics required for open-ended generation.
...
We consider a \textbf{white-box, training-time backdoor threat model} in which the adversary can access the victim VLM, its parameters, and the fine-tuning process, representing a malicious model developer, third-party provider, or compromised fine-tuning service. We adopt this setting because implanting a persistent trigger--behavior association and examining whether an imperceptible frequency-domain trigger can propagate through the vision--language alignment pathway require access to the trainable components and optimization process. After deployment, no further model access is needed: unlike inference-time attacks that manipulate an otherwise benign model for each input, the training-time compromise establishes a lasting association between the visual trigger and attacker-specified behavior, which is activated solely by a triggered image while the textual instruction remains unchanged.
% \begin{equation}
...
\begin{figure*}[!t]
    ...
    InstructBLIP, and LLaVA. Rows correspond to Spectral Signatures,
    Beatrix, and STRIP, while columns correspond to the victim VLMs.
...
\begin{table*}[!t]
\centering
\caption{Ablation Study on Frequency Band}
...
Despite the effectiveness of \textsc{FreqDoor}, this study has two main limitations. First, it assumes a white-box, training-time threat model, where the attacker can access the fine-tuning process and inject poisoned samples. Although this setting is relevant to compromised training pipelines or malicious model providers, it does not cover black-box or test-time attacks. Second, the attack depends on the selected frequency configuration: the frequency band and mixing ratio affect both backdoor activation and perceptual similarity. Lower-frequency components generally produce stronger trigger activation, while larger trigger-source contributions may reduce stealthiness. Future work can explore adaptive or learnable frequency selection to better balance attack effectiveness and imperceptibility under more restrictive attacker settings.
